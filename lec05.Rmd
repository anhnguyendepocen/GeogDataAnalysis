...
To get the workspace, right-click on this link [[geog495.RData]](http://geog.uoregon.edu/GeogR/data/Rdata/geog495.RData) and save it to your working folder.  (If you've forgotten where that is, type `getwd()` on the command line).  Then read it in to R:
...
```{r plot1, fig.width=4, fig.asp=1, message=FALSE}
attach(specmap)
...
example(rgl.spheres)
```
![rgl01](http://geog.uoregon.edu/GeogR/figures/rgl01.jpg)
[[Back to top]](lec05.html)
...
#Lattice-like plots of maps (`spplot()`)#
...
The top panel shows unglaciated cirques in pink and glaciated ones in turquoise, while the bottom panel shows the reverse, glaciated cirques in pink, unglaciated in turquoise.  Note the `aspect` argument --  this scales the horizontal and vertical axes of the plot in a way that makes the map look projected.
...
- [Trellis Graphics User Manual](http://geog.uoregon.edu/GeogR/pdfs/trellis.user.pdf), and
- [A Tour of Trellis Graphics](http://geog.uoregon.edu/GeogR/pdfs/Trellis_tour.pdf)  
