...
- [fitting a plane using two predictor variables](http://geog.uoregon.edu/GeogR/images/nwk7_1.gif)
- [one data point and its deviation from the regression plane or response surface](http://geog.uoregon.edu/GeogR/images/mreg.gif)
...
- [matrix algebra](http://geog.uoregon.edu/GeogR/topics/matrix.pdf)
- [regression analysis in matrix algebra terms](http://geog.uoregon.edu/GeogR/topics/matreg.pdf)

The following example provide a short illustration of the use of matrix algebra to obtain the regression coefficients.  
The  example data set for illustrating the use of regression diagnostics [[regrex3.csv]](http://geog.uoregon.edu/GeogR/data/csv/regrex3.csv) is used here, in particular, the multiple regression using `x1` and `x2` as predictors for the response variable `y5`
...
- illustrations using repeated simulations of data sets with built-in assumption violations [[examples]](http://geog.uoregon.edu/GeogR/images/violate1.gif), [[solutions]](http://geog.uoregon.edu/GeogR/images/violate2.gif)
...
An example data set for illustrating the use of regression diagnostics [[regrex3.csv]](http://geog.uoregon.edu/GeogR/data/csv/regrex3.csv)
First, take a look at the different variables in the example data set.
...
Here are the data [[midwtf2]](http://geog.uoregon.edu/GeogR/data/csv/midwtf2.csv), and the components of a shape file for mapping the data and results [[.dbf]](http://geog.uoregon.edu/GeogR/data/shp/midwotl.dbf)  [[.shx]](http://geog.uoregon.edu/GeogR/data/shp/midwotl.shx)  [[.shp]](http://geog.uoregon.edu/GeogR/data/shp/midwotl.shp)
...
- [Moran statistic details](http://geog.uoregon.edu/GeogR/topics/moran.pdf)

To illustrate the Moran statistic, and gain an impression of the level of spatial autocorrelation in the response (as opposed to the residuals) calculate the Moran statistic for July temperature, the response.  First, map the July temperature variables:
...
points(longitud, latitude, pch=ifelse(plotvar > 0,24,25),
    bg=color, cex=1.5)
legend(-95.0, 43.25, legend=leglabs(brks), fill=plotclr, cex=0.6, bty="n")
```
...
oldpar <- par(mfrow = c(2, 2))
plot(tmeanjul.lm0, which=c(1,2,4,5))
par(oldpar)
```
...
oldpar <- par(mfrow = c(2, 2))
plot(tmeanjul.lm1, which=c(1,2,4,5))
par(oldpar)
```
```{r mr15}
# map residuals
...
points(longitud, latitude, pch=ifelse(plotvar > 0,24,25),
    bg=color, cex=1.5)
legend(-95.0, 43.25, legend=leglabs(brks), fill=plotclr, cex=0.6,
    bty="n")
```
```{r mr16}
# Moran test
...
Here are the shapefile components, which include the data:  [[.dbf]](http://geog.uoregon.edu/GeogR/data/shp/scand.dbf) [[.shx]](http://geog.uoregon.edu/GeogR/data/shp/scand.shx) [[.shp]](http://geog.uoregon.edu/GeogR/data/shp/scand.shp)
...
First, get a reference map of the commune names:

```{r dum03}
# get coordinates of district centroids
scand_prov.pts <- coordinates(scand_prov.shp)
District <- as.character(scand_prov.shp@data$District)
District
```

In this shapefile, the Scandinavian characters in the district names were represented by "escape sequences" which are intended to replace, for example, "ö" with "o".  However, these escape sequences mess up R's ability to display the names on a plot using the `text()` function.  The names can be restored by assignment:  

```{r replace_names}
scand_prov.shp@data$District <- c(
  "Lappi",       "Vaasa",            "Keski Suomi",    "Oulu",        "Kuopio",            "Mikkeli",     "Pohjois Karjala",
  "Hame",        "Uusimaa",          "Turku ja Pori",  "Ahvenanmaa",  "Kymi",              "Norrbotten",  "Västerbotten",
  "Jämtland",    "Västernorrland",   "Gävleborg",      "Uppsala",     "Stockholm",         "Kopparberg",  "Västmanland",
  "Värmland",    "Södermanland",     "Orebro",         "Skaraborg",   "Ostergötland",      "Kalmar",      "Göteborg",
  "Alvsborg",    "Jönköping",        "Blekinge",       "Halland",     "Kronoberg",         "Malmöhus",    "Kristianstad",
  "Gotland",     "Finnmark",         "Troms",          "Nordland",    "Nord-Trondelag",    "Rogaland",    "Hordaland",
  "Vest-Agder",  "More og Romsdal",  "Sor-Trondelag",  "Aust-Agder",  "Sogn og Fjordane",  "Oppland",     "Telemark",
  "Buskerud",    "Ostfold",          "Hedmark",        "Vestfold",    "Akershus",          "Oslo" )
```
Revised names:  
```{r}
scand_prov.shp@data$District
```
...
# plot the names
plot(scand_prov.shp, bor="gray90", axes=TRUE)
points(scand_prov.pts, pch=16, cex=0.4)
text(scand_prov.pts, labels=District, cex=0.5)
```

Next, extact the variable values from the shape file, and plot the yes votes.  (The data are the same as those in `scanvote.csv`, but are extracted from the shapefile here to make sure that the individual values are in the same order as the polygons in the shapefile.)
...
# draw the map
plot(scand_prov.shp, col=colorcode, axes=TRUE)
text(scand_prov.pts, labels=Y, cex=.5)
legend("topleft", legend=leglabs(brks), fill=plotclr, cex=0.8, bty="n")
```
...
plot(Y ~ log10(X), ylim=range(Y, pred.int, na.rm=T), type="n")
...
plotclr <- brewer.pal(nclr,"PuOr")
brks <- seq(-8,8,2)
colornum <- findInterval(plotvar, brks, all.inside = TRUE)
colorcode <- plotclr[colornum]

plot(scand_prov.shp, col=colorcode)
points(scand_prov.pts, type="n")
text(scand_prov.pts, labels=as.character(round(plotvar, 1)), cex=.5)
legend("topleft", legend=leglabs(brks), fill=plotclr, cex=0.8, bty="n")
```
...
plot(residuals(vote.lm1) ~ Country, ylim=c(-15,15))
par <- opar
```
...
lines(log10(X)[Country=="N"],fitted(vote.lm2)[Country=="N"], lwd=2, col="red")
lines(log10(X)[Country=="F"],fitted(vote.lm2)[Country=="F"], lwd=2, col="blue")
lines(log10(X)[Country=="S"],fitted(vote.lm2)[Country=="S"], lwd=2, col="purple")
...
# residual grouped boxplot
plot(residuals(vote.lm1) ~ Country, ylim=c(-15,15))
plot(residuals(vote.lm2) ~ Country, ylim=c(-15,15))
par <- opar
```
...
lines(log10(X)[Country=="N"],fitted(vote.lm3)[Country=="N"], lwd=2, col="red")
lines(log10(X)[Country=="F"],fitted(vote.lm3)[Country=="F"], lwd=2, col="blue")
lines(log10(X)[Country=="S"],fitted(vote.lm3)[Country=="S"], lwd=2, col="purple")
...
```{r dum17}
# map residuals
plotvar <- vote.lm3$residuals # gets residuals
nclr <- 8
plotclr <- brewer.pal(nclr,"PuOr")
brks <- seq(-8,8,2)
colornum <- findInterval(plotvar, brks, all.inside = TRUE)
...
plot(residuals(vote.lm1) ~ Country, ylim=c(-15,15))
plot(residuals(vote.lm2) ~ Country, ylim=c(-15,15))
plot(residuals(vote.lm3) ~ Country, ylim=c(-15,15))
par <- opar
```
